...
def camera_matrix(eye,target,w,h):
 ...
 tan=np.tan(np.deg2rad(68/2));near=.02;far=700
 proj=np.array([[1/tan,0,0,0],[0,w/h/tan,0,0],[0,0,-(far+near)/(far-near),-2*far*near/(far-near)],[0,0,-1,0]],float)
 return proj@view

def main():
 ap=argparse.ArgumentParser();ap.add_argument('--gles',action='store_true');ap.add_argument('--resolved',default='resolved');ap.add_argument('--width',type=int,default=1200);ap.add_argument('--height',type=int,default=800);ap.add_argument('--film',action='store_true');ap.add_argument('--output',default='evidence/formation');a=ap.parse_args();D=R/a.resolved
 (O/a.output).mkdir(parents=True,exist_ok=True)
 ...
 poses=[('hero',[-.42,-5.8,1.56],[.15,9.8,3.12]),('forward',[.25,4.8,1.60],[1.2,16,2.6]),('reverse',[.6,9.0,1.75],[-.3,-4,2.6]),('orbit',[-63,-55,62],[2,16,3]),('rear',[65,74,52],[3,19,4]),('rim',[-10,-4,20],[1,17,7])]
 if a.film:
  poses=[]
  for frame in range(144):
   t=frame/71 if frame<72 else (frame-72)/71
   if frame<72:
    eye=[-.42+.12*np.sin(t*2*np.pi),-5.8+t*2.1,1.56+.015*np.sin(t*2*np.pi)];target=[.15,9.8+t*2.1,3.12]
   else:
    az=-2.45+t*.80;eye=[2+100*np.cos(az),16+100*np.sin(az),57+7*np.sin(t*np.pi)];target=[2,16,3]
   poses.append((f'{frame:04d}',eye,target))
 results=[]
 for name,eye,target in poses:
  for mode,label in [(0,'beauty')]+([(1,'direct'),(2,'indirect'),(3,'normals'),(4,'visibility')] if name=='hero' and not a.film else []):
   ...
   path=O/a.output/(f'{name}.png' if a.film else f'{name}_{label}.png');Image.fromarray(im).convert('RGB').save(path)
 ...
 (O/a.output/'render_execution.json').write_text(json.dumps({'renderer':g.GetString(0x1F01).decode(),'version':g.GetString(0x1F02).decode(),'same_shader_source_as_threejs':True,'glsl_version':'300 es' if a.gles else '330 core','shader_sha256':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (O/'source').glob('*.glsl')},'browser_execution':False,'width':a.width,'height':a.height,'seconds':time.monotonic()-start,'results':results},indent=2))
