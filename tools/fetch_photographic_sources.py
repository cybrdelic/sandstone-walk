"""Fetch unchanged CC0 Poly Haven images and fingerprint their provenance."""
...
KINDS={'diff':['Diffuse','diff'],'nor_gl':['nor_gl'],'rough':['Rough','rough'],'ao':['AO','ao'],'disp':['Displacement','disp']}
HEADERS={'User-Agent':'SandstoneWalk/0.5 (https://github.com/cybrdelic/sandstone-walk)'}
def get(url):
    error=None
    for attempt in range(4):
        try:
            with urllib.request.urlopen(urllib.request.Request(url,headers=HEADERS),timeout=90) as r:return r.read()
        except Exception as e:error=e;time.sleep(attempt+1)
    raise RuntimeError(f'Could not fetch {url}: {error}')
def main():
    OUT.mkdir(parents=True,exist_ok=True)
    report={'schema':'sandstone-walk-photographic-intake/1','license':'CC0-1.0','license_source':'https://polyhaven.com/license','assets':{}}
    for asset,tile in ASSETS.items():
        folder=OUT/asset;folder.mkdir(exist_ok=True)
        raw=get('https://api.polyhaven.com/files/'+asset);(folder/'files.json').write_bytes(raw);files=json.loads(raw)
        ...
        for kind,aliases in KINDS.items():
            key=next((k for k in aliases if k in files),None)
            variants=files.get(key,{}).get('4k',{})
            if not variants:raise RuntimeError(f'No 4K {kind} map: {asset}; available: {list(files)}')
            ext='png' if 'png' in variants else 'jpg'
            record=variants[ext];url=record['url'];data=get(url);path=folder/(kind+'.'+ext);path.write_bytes(data)
