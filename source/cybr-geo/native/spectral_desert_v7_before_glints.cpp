...
V whiteRGB, SUN=unit(V(-.48f,.86f,.26f));
...
float poolQ(V p,int i){
    ...
    float edge=1+.18f*std::sin(3*ang+.55f)+.090f*std::sin(2*ang-1.3f)+.038f*std::sin(7*ang+.9f*i)
...
constexpr float STEAM_SCALE=.86f,MAJORANT=.85f,PHASE_G=.63f;
...
const V ATM_LO(-4500,330,-.4f),ATM_HI(4500,7000,1200);
constexpr float ATM_SIGMA=.000042f;
...
Surface shade(V p,V &n,int material,float footprint){
    ...
    float wet=(1-smooth(level+.014f,level+.12f+.022f*noise(p*7),p.z))*(1-smooth(1.2f,1.52f,q));
    float mineral=std::exp(-sqr(d/.46f));
    float ff=1/(1+sqr(footprint*200)),normalStrength=.09f;
    if(material==0||material==4){
        V sand(.245f,.206f,.164f),carbonate(.76f,.726f,.627f);
        float deposition=clamp(mineral*(.93f+.75f*patch)+.13f*noise(p*7));
        m.color=sand*(1-deposition)+carbonate*deposition;
        float lithology=smooth(-.22f,.37f,fbm(V(p.x*.77f,p.y*.81f,p.z*2)));
        m.color=m.color*(.76f+.30f*lithology);
        if(q<.98f){
            float sediment=smooth(-.19f,.39f,fbm(V(p.x*.9f,p.y*1.05f,p.z*1.7f)));
            m.color=V(.61f,.645f,.617f)*(.53f+.59f*sediment)*(1+.25f*meso);
        }
        float iron=std::exp(-sqr((q-1.018f)/.12f))*smooth(-.28f,.36f,fbm(V(p.x*.72f,p.y*2.1f,p.z*5.8f)));
        m.color=m.color*(1-iron*.68f)+V(.26f,.105f,.026f)*(iron*.68f);
        if(material==4)m.color=V(.61f,.585f,.506f)*(1+.28f*meso+.12f*macro)*(1-.26f*iron);
        m.color*=1+.19f*macro+.13f*meso+.055f*grain*ff;
        m.rough=.88f*(1-wet)+.18f*wet;
        if(!photographicGrain.levels.empty()){
            float u=p.x*.917f+p.y*.399f,v=p.y*.917f-p.x*.399f;
            float g=photographicGrain.sample(u,v,footprint,.66f);
            float amount=(material==4?.53f:.93f)*(q<1?.65f:1.f);
            m.color*=1-amount+amount*clamp(g,.21f,2.5f);
        }
        normalStrength=.045f;
    }else if(material==1||material==2){
        float litho=smooth(-.24f,.39f,fbm(p*.83f));
        V grey(.175f,.168f,.150f),tan(.33f,.263f,.179f);
        m.color=material==1?V(.43f,.415f,.367f):(grey*(1-litho)+tan*litho);
        float quartz=smooth(.14f,.52f,noise(p*247.1f))*ff;
        float biotite=smooth(.04f,.42f,noise(p*316.3f+V(8,3,1)))*ff;
        float feldspar=smooth(.12f,.42f,noise(p*98.3f+V(3,8,11)));
        m.color*=1+.42f*macro+.27f*meso;
        m.color=m.color*(1-.42f*quartz)+V(.71f,.685f,.603f)*(.42f*quartz);
        m.color*=1-.42f*biotite;
        m.color=m.color*(1-.27f*feldspar)+V(.30f,.164f,.084f)*(.27f*feldspar);
        float bed=p.z*41+p.x*13-p.y*4+.65f*fbm(p*4);
        m.color*=.68f+.32f*smooth(.014f,.17f,std::fabs(std::sin(bed)));
        float crust=mineral*smooth(-.1f,.50f,patch)*smooth(-.02f,.69f,n.z)*.61f;
        m.color=m.color*(1-crust)+V(.65f,.619f,.513f)*crust;
        if(!photographicGrain.levels.empty()){
            V weight(n.x*n.x,n.y*n.y,n.z*n.z);
            float grit=weight.x*photographicGrain.sample(p.y,p.z,footprint,.27f)
                      +weight.y*photographicGrain.sample(p.x,p.z,footprint,.27f)
                      +weight.z*photographicGrain.sample(p.x,p.y,footprint,.27f);
            m.color*=.48f+.52f*clamp(grit,.23f,2.6f);
        }
        m.rough=.79f*(1-wet)+.23f*wet;normalStrength=.23f;
    }else if(material==3){
        m.color=V(.33f,.238f,.110f)*(1+.37f*macro+.23f*noise(p*47));m.rough=.91f;
    }else if(material==5){
        float strata=std::sin(p.z*.48f+p.x*.061f+fbm(p*.035f)*2.2f);
        float exposure=smooth(.055f,.43f,1-n.z);
        float geology=smooth(-.24f,.32f,fbm(V(p.x*.024f,p.y*.021f,p.z*.034f)));
        V talus(.24f,.212f,.176f),rock(.089f,.090f,.089f);
        float exposed=clamp(exposure*.87f+.36f*geology);
        m.color=(talus*(1-exposed)+rock*exposed)*(1+.31f*fbm(p*.65f)+.14f*noise(p*2.8f)+.12f*strata);
        float ramp=smooth(350,900,p.y);
        m.color=m.color*ramp+V(.255f,.223f,.178f)*(1-ramp)*(.82f+.16f*fbm(p*.59f));
        m.rough=.93f;normalStrength=.24f;
    }else if(material==8){
        m.color=V(.115f,.087f,.052f)*(1+.36f*macro+.21f*noise(p*79));m.rough=.94f;
    }else if(material==9){
        float dry=smooth(-.26f,.35f,noise(p*7.3f));
        m.color=V(.147f,.165f,.087f)*(1-dry)+V(.259f,.235f,.133f)*dry;
        m.color*=1+.31f*macro+.17f*noise(p*68);m.rough=.90f;
    }else{m.color=V(.3f);}
    m.color*=1-.34f*wet;m.ior=(p.z<level&&q<1.1f)?1.48f/WATER_IOR:1.48f;
    m.color=vmin(vmax(m.color,V(.003f)),V(.88f));
    if(material==0||material==1||material==2||material==4||material==5){
        Frame frame(n);float f=material==5?.51f:48.f,filter=1/(1+sqr(footprint*f));
        V u=frame.t*.013f,b=frame.b*.013f,pf=p*f;
        float dt=(noise(pf+u)-noise(pf-u))/.026f,db=(noise(pf+b)-noise(pf-b))/.026f;
        n=unit(n-(frame.t*dt+frame.b*db)*(normalStrength*filter));
        float f2=330,filter2=1/(1+sqr(footprint*f2));
        n=unit(n+(frame.t*noise(p*f2)+frame.b*noise(p*f2+V(5,8,2)))*(.105f*filter2));
    }
    return m;
}
...
constexpr float WATER_SIGMA_S=.022f;
...
int main(int argc,char**argv){try{
    ...
    V camera(2.45f,-4.35f,1.28f),target(.45f,3.4f,.21f);float hfov=73;
    if(o.view=="detail"){camera=V(4.95f,-2.65f,.82f);target=V(1.40f,1.75f,.13f);hfov=62;}
