...
float poolQ(V p,int i){
    const auto&k=pools[i];float dx=(p.x-k.x)/k.rx,dy=(p.y-k.y)/k.ry,a=std::atan2(dy,dx);
    float edge=1+.13f*std::sin(3*a+.55f)+.065f*std::sin(2*a-1.3f)+.044f*std::sin(7*a+.9f*i);
    edge+=.10f*noise(p.x*.55f,p.y*.55f)+.030f*noise(p.x*2.7f,p.y*2.7f);
    float q=std::sqrt(dx*dx+dy*dy)/edge;
    if(i==0)q+=.18f*std::exp(-sqr((p.x-3.0f)/1.3f)-sqr((p.y+.55f)/1.0f));
    return q;
}
...
#include "exponential_haze_v10.h"
...
#include "landscape_materials_v11.h"
...
#include "specular_reflection_connection.h"
    ...
    for(int depth=0;depth<maxDepth;depth++){
        ...
        if(useWater&&waterPool<0){
            for(int pool=0;pool<2;pool++){
                ...
                Spec bsdf=foliage?base*((cosine>=0?.60f:.40f)/PI):evalBSDF(m,base,n,v,c.l);
        ...
        if((material==3||material==9)&&waterPool<0){
            ...
            if(cosine>0&&!opaqueShadow(Ray(p+offset*EPS*5,lAir),INF)){
                ...
                recordRadiance(L,throughput*base*solar*(prob*cosine*SUN_SOLID*tr/PI),indirect);
            }
...
struct Options{
    std::string mesh,out,grain,relief,view="hero";int width=1200,height=800,spp=128,depth=12,threads=5,seed=20260913;
...
Options parse(int argc,char**argv){
    ...
    for(int i=3;i<argc;i++){
        ...
        if(arg=="--water-spp")o.waterSpp=std::stoi(get());else if(arg=="--indirect-clamp")indirectClamp=std::stof(get());else if(arg=="--w")o.width=std::stoi(get());else if(arg=="--h")o.height=std::stoi(get());else if(arg=="--spp")o.spp=std::stoi(get());
        ...
        else if(arg=="--relief")o.relief=get();else if(arg=="--grain")o.grain=get();else if(arg=="--no-bands")o.bands=false;else throw std::runtime_error("Unknown argument "+arg);
    }
    if(o.width<1||o.height<1||o.spp<1||o.depth<1||o.threads<1||o.aperture<0||waterStrength<0||o.waterSpp<0||o.waterSpp>65535||o.spp>65535||indirectClamp<0)throw std::runtime_error("Invalid render setting");
    return o;
}
...
int main(int argc,char**argv){try{
    ...
    auto o=parse(argc,argv);if(!o.grain.empty())photographicGrain.load(o.grain);if(!o.relief.empty())granularRelief.load(o.relief);omp_set_num_threads(o.threads);initSpectra();initRipples();initClouds();initSteam();
    ...
    for(int y=0;y<o.height;y++){
        for(int x=0;x<o.width;x++){
            ...
            sampleCounts[index]=uint16_t(pixelSpp);
