...
def robust_sky_outliers(rgb, guide, metadata, counts, enabled=True):
    """Reject isolated, high-variance sky spikes in the *display filter only*.

    Raw radiance is never changed. This biased non-neural operation requires a
    5x5 majority of ordinary-budget sky neighbours and an extreme 20x local
    radiance ratio. Water, opaque geometry and the sun-disc neighbourhood are
    excluded. The exact modified-pixel mask is returned for audit.
    """
    mask=np.zeros(rgb.shape[:2],dtype=bool)
    if not enabled:return rgb.copy(),mask
    h,w=mask.shape
    sky=(guide[:,:,8]<0)&(counts==metadata['spp'])
    peak=np.max(rgb,axis=2)
    candidates=sky&(peak>3.)&(guide[:,:,7]>.1)
    yy,xx=np.where(candidates)
    output=rgb.copy()
    camera=np.asarray(metadata['camera_origin_m'],float)
    forward=np.asarray(metadata['camera_target_m'],float)-camera
    forward/=np.linalg.norm(forward)
    right=np.cross(forward,[0.,0.,1.]);right/=np.linalg.norm(right)
    up=np.cross(right,forward)
    sun=np.asarray(metadata['sun_direction'],float);sun/=np.linalg.norm(sun)
    tangent=np.tan(np.deg2rad(metadata['horizontal_fov_degrees']*.5))
    for y,x in zip(yy,xx):
        if x<2 or x>=w-2 or y<2 or y>=h-2:continue
        px=(2*(x+.5)/w-1)*tangent;py=(1-2*(y+.5)/h)*tangent/(w/h)
        direction=forward+right*px+up*py;direction/=np.linalg.norm(direction)
        if np.dot(direction,sun)>.9998:continue
        valid=sky[y-2:y+3,x-2:x+3].copy();valid[2,2]=False
        if int(valid.sum())<20:continue
        neighbourhood=rgb[y-2:y+3,x-2:x+3][valid]
        median=np.median(neighbourhood,axis=0)
        nearby_peaks=np.max(neighbourhood,axis=1)
        threshold=max(3.,20.*max(.01,float(np.max(median))))
        if peak[y,x]>threshold and int(np.count_nonzero(nearby_peaks>threshold))==0:
            output[y,x]=median;mask[y,x]=True
    return output,mask
...
def main():
    ...
    p.add_argument('--exposure',type=float);p.add_argument('--white-balance',type=float,default=5000.);p.add_argument('--sky-passes',type=int,default=3);p.add_argument('--opaque-filter-strength',type=float,default=.35);p.add_argument('--no-sky-despike',action='store_true');a=p.parse_args();stem=a.stem
    ...
    set_num_threads(4)
    filtered,spike_mask=robust_sky_outliers(rgb,guide,metadata,counts,not a.no_sky_despike)
    np.save(Path(str(stem)+'_sky_filter_mask.npy'),spike_mask)
    ...
    audit={
        ...
        'isolated_sky_spike_filter_enabled':not a.no_sky_despike,
        'isolated_sky_spike_pixels':int(spike_mask.sum()),
        'sky_spike_policy':'Biased non-neural display filter only; high-variance isolated 20x local peaks; raw radiance unchanged; water, geometry and sun-disc neighbourhood excluded'}
