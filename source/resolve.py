...
def run(root,baked,out):
 ...
 for entry in layout['parts']:
  ...
  if entry['kind']=='grid':
   ...
   f=np.load(root/f'data/{name}.idx.npy');ef=filter_rocks(e,n,f,p)
 ...
 (out/'resolve.json').write_text(json.dumps({'method':'positive normalized linear-light GI filtering followed by spectral reflectance product and 16-band integration','geometry_modified':False,'albedo_smoothed':False,'direct_color_smoothed':False,'direct_visibility_reconstruction':'one-ring conservative minimum across normals with dot > 0.86; finite shadow-edge erosion; raw ray results unchanged','parts':reports},indent=2))
