// Regenerated-geometry adapter: expected topology is a required compile-time argument.
#ifndef SOLID_TRIANGLE_COUNT
#error SOLID_TRIANGLE_COUNT must come from the validated regenerated layout
#endif
...
void loadExactMesh(const std::string&path){
 ...
 if(!f||n!=SOLID_TRIANGLE_COUNT)throw std::runtime_error("Unexpected geometry count; refusing a substitute scene");
 tris.resize(n);
...
int main(int argc,char**argv){try{
 ...
 for(size_t first=0;first<vertices.size();first+=batch){size_t count=std::min(batch,vertices.size()-first);
  ...
  for(size_t j=0;j<count;j++){
   ...
   Spec base=rgbAnchors(m.color);VertexBake v;v.n=geom;v.rough=m.rough;v.reflectance=base;v.direct=toRGB(base*solar)*(SUN_SOLID/PI);
 ...
 // Linear mapping from clamped spectral anchors to outgoing RGB. No texture
 // colors are included in this matrix: receiver material is evaluated per pixel.
 std::ofstream basis(output+"/anchor_basis.f32",std::ios::binary);
 for(int k=0;k<NBANDS;k++){
  float w=WL0+(k+.5f)*DL,b=clamp((w-465.f)/90.f),r=clamp((w-550.f)/95.f);
  b=b*b*(3-2*b);r=r*r*(3-2*r);V row(r,b*(1-r),1-b);
  basis.write(reinterpret_cast<char*>(&row),sizeof(V));
 }
 std::ofstream calibration(output+"/rgb_to_anchors.f32",std::ios::binary);
 for(V row:RGB_TO_ANCHORS)calibration.write(reinterpret_cast<char*>(&row),sizeof(V));
 std::ofstream sr(output+"/solar_anchor_response.f32",std::ios::binary);
 for(int c=0;c<3;c++){
  Spec a=rawRGBAnchors(c==0?V(.5f,0,0):c==1?V(0,.5f,0):V(0,0,.5f));
  V response=toRGB(a*solar)*(2*SUN_SOLID/PI);sr.write(reinterpret_cast<char*>(&response),sizeof(V));
 }
